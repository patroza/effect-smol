...
interface InternalStartOptions extends Actor.StartOptions {
...
const makeActorRuntime: Effect.Effect<ActorRuntime> = Effect.gen(function*() {
  ...
  function spawn<ChildState, ChildEvent, ChildError, ChildRequirements, ChildOutput>(
    logic: Actor.ActorLogic<ChildState, ChildEvent, ChildError, ChildRequirements, ChildOutput>,
    spawnOptions?: Actor.SpawnOptions
  ): SpawnResult<
    ChildState,
    ChildEvent,
    ChildError,
...
) => Effect.Effect<Actor.Actor<State, Event, Error, Output>, ActorSystemIdAlreadyExistsError, Requirements> = Effect
  .fnUntraced(
